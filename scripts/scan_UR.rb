...
db.prepare('insert_mp','insert into mp (id,resolved_by,resolved_on,weight,position,resolution,type_mp) values ($1,$2,$3,$4,ST_SetSRID(ST_Point($5, $6), 4326),$7,$8)')
db.prepare('insert_ur',"insert into ur (id,position,resolved_by,resolved_on,created_on,resolution,type_ur) values ($1,ST_SetSRID(ST_Point($2, $3), 4326),$4,$5,$6,$7,$8)")
  ...
  while lonStart < longEast do
    ...
    while latStart > latSouth do
      ...
      begin
        ...
        #puts "[#{area.join(',')}] URs ok"

        json = JSON.parse(wme.body)
        #puts "#{json}\n===" if json['mapUpdateRequests']['objects'].size > 0
        #exit
        ...
        json['problems']['objects'].each do |m|
          begin
            db.exec_prepared('insert_mp',[m['id'][2..-1], m['resolvedBy'], (m['resolvedOn'].nil? ? nil : Time.at(m['resolvedOn']/1000)), m['weight'], m['geometry']['coordinates'][0], m['geometry']['coordinates'][1], m['resolution'], m['type']]) if db.exec_params('select id from mp where id = $1',[m['id'][2..-1]]).count == 0
...
#          if urs_area.size > 0
#            agent.cookie_jar.clear!
#            puts "Buscando URs #{urs_area.join(',')}"
...
#            puts "[#{area.join('%2C')}] dados URs ok"
#
...
#                #puts "#{u}" if u.has_key?('comments') and u['comments'].size > 0
...
#          end
#        rescue Mechanize::ResponseCodeError
#          puts "Erro buscando URs #{urs_area.join(',')}"
#        end
